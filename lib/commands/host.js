...
module.exports = function(varPath, port, host, program) {
  //Start Server
  var app = express();
  var localController;
  ...
  try {
    localController = new LocalController(variation, port, host);
  } catch(error) {
    logger.log("error", error.message);
    return;
  }
  ...
  var onStartup = function(){
    if(!program.silence){
      console.log("Serving variation " + varPath + " at " + host + ":" + localController.port);
      console.log("point your browser to http" + (program.ssl ? "s" : "") + "://" + host + ":" + localController.port);
  ...
  if (program.ssl) {
    ...
    return app
      .listen(localController.port, host, onStartup)
      .on('error', handleStartupError);
  }
};
